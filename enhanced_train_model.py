#!/usr/bin/env python3
"""
Enhanced Space Weather Forecaster with Comprehensive Feature Integration
========================================================================

This enhanced version incorporates data from all available CSV files to create
a more comprehensive space weather prediction model that considers multiple
factors beyond just geostorm detection.

Key Improvements:
1. Integration of data from all 8 CSV files
2. Advanced feature engineering using all available space weather parameters
3. Multi-factor storm prediction (not just geostorm detection)
4. Enhanced temporal feature engineering
5. Improved class balancing and hyperparameter optimization
"""
...
print("="*80)
print("ENHANCED SPACE WEATHER FORECASTER - COMPREHENSIVE FEATURE INTEGRATION")
print("="*80)
...
# Alert features
if 'is_storm_alert_active' not in df.columns:
    # Create synthetic alert feature if not available
    if 'noaa_dst' in df.columns and 'noaa_kp' in df.columns:
        df['is_storm_alert_active'] = ((df['noaa_dst'] < -50) | (df['noaa_kp'] >= 5)).astype(int)
    else:
        df['is_storm_alert_active'] = 0
...
for feature in base_features:
    if feature in df.columns:
        df[f'{feature}_diff_1h'] = df[feature].diff()
        df[f'{feature}_diff_3h'] = df[feature].diff(3)
        df[f'{feature}_pct_change_1h'] = df[feature].pct_change().fillna(0)
...
def objective(trial):
    ...
    # Custom scoring that considers all classes
...
print("Optimizing for overall accuracy across all space weather conditions...")
...
# =============================================================================
# 7. SAVE ENHANCED MODEL AND RESULTS
# =============================================================================
print("\n7. SAVING ENHANCED MODEL AND RESULTS")
print("-" * 50)

# Save model
final_model.save_model('enhanced_space_weather_forecaster.cbm')
print("✅ Enhanced model saved as 'enhanced_space_weather_forecaster.cbm'")

# Save feature importance
importance_df.to_csv('enhanced_feature_importance.csv', index=False)
print("✅ Feature importance saved as 'enhanced_feature_importance.csv'")

# Save optimization results
study_df = study.trials_dataframe()
study_df.to_csv('enhanced_hyperparameter_optimization_results.csv', index=False)
print("✅ Optimization results saved as 'enhanced_hyperparameter_optimization_results.csv'")
...
results_df.to_csv('enhanced_model_test_results.csv', index=False)
print("✅ Test results saved as 'enhanced_model_test_results.csv'")
...
print(f"   ✓ Advanced temporal feature engineering")
...
print(f"\n📁 Files Created:")
print(f"   - enhanced_space_weather_forecaster.cbm")
print(f"   - enhanced_feature_importance.csv")
print(f"   - enhanced_hyperparameter_optimization_results.csv")
print(f"   - enhanced_model_test_results.csv")

print(f"\n🚀 Ready for production use with comprehensive space weather prediction!")
print("="*80)